...
public class EligibiliteServiceImpl implements EligibiliteService {
  ...
  public EligibiliteResult verifierEligibilite(
    ...
    if (banque == null
        || banque.getMandatStatut() != MandatStatut.VALIDE
        || !banque.isMandatSigne()) {
  ...
  private EligibiliteResult calculerEligibiliteFinanciere(InscriptionAnnuelle ins) {
    return switch (ins.getNiveau()) {
      ...
      case L2_ANNEE ->
          verifierCredits(
              ins.getCreditsTotalValides(),
              TypeParametreDbs.L2_CREDITS_MIN_STANDARD,
              TypeParametreDbs.L2_CREDITS_MIN_SUPERIEUR);
      case L3_ANNEE ->
          verifierCredits(
              ins.getCreditsTotalValides(),
              TypeParametreDbs.L3_CREDITS_MIN_STANDARD,
              TypeParametreDbs.L3_CREDITS_MIN_SUPERIEUR);
      case L4_ANNEE ->
          verifierCreditsRecyclage(
              ins.getCreditsTotalValides(), TypeParametreDbs.L4_CREDITS_MIN_STANDARD);
      case L5_ANNEE ->
          verifierCreditsRecyclage(
              ins.getCreditsTotalValides(), TypeParametreDbs.L5_CREDITS_MIN_STANDARD);
  ...
  private EligibiliteResult verifierL1(BigDecimal moyenne) {
    ...
    BigDecimal minPassable =
        regleBourseService.getValeurAsBigDecimal(TypeParametreDbs.L1_MOYENNE_MIN_PASSABLE);
    BigDecimal minMentionSup =
        regleBourseService.getValeurAsBigDecimal(TypeParametreDbs.L1_MOYENNE_MIN_MENTION_SUP);

    if (moyenne.compareTo(minMentionSup) >= 0) {
      return EligibiliteResult.eligible(
          regleBourseService.getValeurAsBigDecimal(TypeParametreDbs.MONTANT_TRANCHE_SUPERIEURE));
    } else if (moyenne.compareTo(minPassable) >= 0) {
      return EligibiliteResult.eligible(
          regleBourseService.getValeurAsBigDecimal(TypeParametreDbs.MONTANT_TRANCHE_STANDARD));
    }
    return EligibiliteResult.nonEligible("Moyenne insuffisante pour L1");
  }

  private EligibiliteResult verifierCredits(
      int credits, TypeParametreDbs regleMin, TypeParametreDbs regleSup) {
    ...
    return EligibiliteResult.nonEligible(
        String.format("Crédits insuffisants (%d < %d)", credits, minStandard));
  }
  ...
  private EligibiliteResult verifierCreditsRecyclage(int credits, TypeParametreDbs regleMin) {
    ...
    return EligibiliteResult.nonEligible(
        String.format("Crédits insuffisants pour recyclage (%d < %d)", credits, min));
  }
}
