...
public class DashboardServiceImpl implements DashboardService {
  ...
  public Map<String, Object> getGlobalStats() {
    ...
    long totalEligibles =
        studentRepository.countByStatutKYC(com.backend.gns.core.domain.enums.KycStatus.VALIDEE);
    long totalPending =
        studentRepository.countByStatutKYC(com.backend.gns.core.domain.enums.KycStatus.EN_ATTENTE);
    long totalRejected =
        studentRepository.countByStatutKYC(com.backend.gns.core.domain.enums.KycStatus.REJETE);
    stats.put("totalEligibles", totalEligibles);
    stats.put("totalPending", totalPending);

    double verificationRate =
        totalStudents > 0 ? ((totalEligibles + totalRejected) * 100.0 / totalStudents) : 0.0;
    stats.put(
        "verificationRate",
        Double.parseDouble(String.format(java.util.Locale.US, "%.1f", verificationRate)));

    java.time.LocalDateTime startOfToday =
        java.time.LocalDateTime.now().withHour(0).withMinute(0).withSecond(0).withNano(0);
    java.time.LocalDateTime endOfToday =
    ...
    BigDecimal volumeToday =
        paiementRepository.sumMontantDebiteBetweenDatesAndStatut(
            startOfToday, endOfToday, com.backend.gns.paiement.domain.enums.PaiementStatut.VALIDE);
    stats.put("volumeToday", volumeToday != null ? volumeToday : BigDecimal.ZERO);

    Long failedTxns =
        paiementRepository.countByStatutPaiement(
            com.backend.gns.paiement.domain.enums.PaiementStatut.ANNULEE);
    stats.put("failedTxns", failedTxns != null ? failedTxns : 0L);

    Long successTxns =
        paiementRepository.countByStatutPaiement(
            com.backend.gns.paiement.domain.enums.PaiementStatut.VALIDE);
    double successRate = totalTx > 0 ? (successTxns * 100.0 / totalTx) : 0.0;
    stats.put(
        "successRate", Double.parseDouble(String.format(java.util.Locale.US, "%.2f", successRate)));
  ...
  public List<Map<String, Object>> getFluxMensuel() {
    ...
    String[] mois = {
      "Jan", "Fév", "Mar", "Avr", "Mai", "Jun", "Juil", "Août", "Sep", "Oct", "Nov", "Déc"
    };
