...
public class CommandeLigneMapper {
    ...
    public CommandeLigneMapper(CommandeRepository commandeRepository, 
    ...
    public CommandeLigne toEntity(CommandeLigneRequest request) {
        ...
        if (request.trackingCommandeId() != null) {
            Commande commande = commandeRepository.findByTrackingId(request.trackingCommandeId())
                .orElseThrow(() -> new IllegalArgumentException("Commande non trouvée avec l'ID: " + request.trackingCommandeId()));
            commandeLigne.setCommande(commande);
        }

        if (request.trackingProductId() != null) {
            Product product = productRepository.findByTrackingId(request.trackingProductId())
                .orElseThrow(() -> new IllegalArgumentException("Produit non trouvé avec l'ID: " + request.trackingProductId()));
    ...
    public CommandeLigneResponse toResponse(CommandeLigne commandeLigne) {
        ...
        return CommandeLigneResponse.builder()
                ...
                .trackingCommandeId(commandeLigne.getCommande() != null ? commandeLigne.getCommande().getTrackingId() : null)
                .trackingProductId(commandeLigne.getProduct() != null ? commandeLigne.getProduct().getTrackingId() : null)
                .build();
    }
    ...
    public CommandeLigne toEntityFromResponse(CommandeLigneResponse response) {
        ...
        if (response.trackingCommandeId() != null) {
            Commande commande = commandeRepository.findByTrackingId(response.trackingCommandeId())
                .orElseThrow(() -> new IllegalArgumentException("Commande non trouvée avec l'ID: " + response.trackingCommandeId()));
            commandeLigne.setCommande(commande);
        }

        if (response.trackingProductId() != null) {
            Product product = productRepository.findByTrackingId(response.trackingProductId())
                .orElseThrow(() -> new IllegalArgumentException("Produit non trouvé avec l'ID: " + response.trackingProductId()));
            commandeLigne.setProduct(product);
        }
