...
public class CommandeMapper {
    ...
    public CommandeMapper(StudentRepository studentRepository, 
    ...
    public Commande toEntity(CommandeRequest request) {
        ...
        commande.setDateCommande(request.dateCommande() != null ? request.dateCommande() : LocalDateTime.now());
        ...
        if (request.studentTrackingId() != null) {
            Student student = studentRepository.findByTrackingId(request.studentTrackingId())
                .orElseThrow(() -> new IllegalArgumentException("Étudiant non trouvé avec l'ID: " + request.studentTrackingId()));
    ...
    public CommandeResponse toResponse(Commande commande) {
        ...
        return CommandeResponse.builder()
                ...
                .merchantTrackingId(commande.getMerchant() != null ? commande.getMerchant().getTrackingId() : null)
                .build();
    }
    ...
    public Commande toEntityFromResponse(CommandeResponse response) {
        ...
        if (response.studentTrackingId() != null) {
            Student student = studentRepository.findByTrackingId(response.studentTrackingId())
                .orElseThrow(() -> new IllegalArgumentException("Étudiant non trouvé avec l'ID: " + response.studentTrackingId()));
            commande.setStudent(student);
        }
