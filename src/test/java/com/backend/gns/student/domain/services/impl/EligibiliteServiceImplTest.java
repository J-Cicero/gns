...
class EligibiliteServiceImplTest {
  ...
  @Mock private ParametreDbsService regleBourseService;
  ...
  void setup() {
    ...
    lenient()
        .when(regleBourseService.getValeurAsInteger(TypeParametreDbs.AGE_MAX_LICENCE))
        .thenReturn(26);
    lenient()
        .when(regleBourseService.getValeurAsBigDecimal(TypeParametreDbs.MONTANT_TRANCHE_STANDARD))
        .thenReturn(MONTANT_STD);
    lenient()
        .when(regleBourseService.getValeurAsBigDecimal(TypeParametreDbs.MONTANT_TRANCHE_SUPERIEURE))
        .thenReturn(MONTANT_SUP);

    lenient()
        .when(regleBourseService.getValeurAsBigDecimal(TypeParametreDbs.L1_MOYENNE_MIN_PASSABLE))
        .thenReturn(new BigDecimal("10"));
    lenient()
        .when(regleBourseService.getValeurAsBigDecimal(TypeParametreDbs.L1_MOYENNE_MIN_MENTION_SUP))
        .thenReturn(new BigDecimal("12"));

    lenient()
        .when(regleBourseService.getValeurAsInteger(TypeParametreDbs.L2_CREDITS_MIN_STANDARD))
        .thenReturn(25);
    lenient()
        .when(regleBourseService.getValeurAsInteger(TypeParametreDbs.L2_CREDITS_MIN_SUPERIEUR))
        .thenReturn(54);

    lenient()
        .when(regleBourseService.getValeurAsInteger(TypeParametreDbs.L3_CREDITS_MIN_STANDARD))
        .thenReturn(50);
    lenient()
        .when(regleBourseService.getValeurAsInteger(TypeParametreDbs.L3_CREDITS_MIN_SUPERIEUR))
        .thenReturn(108);

    lenient()
        .when(regleBourseService.getValeurAsInteger(TypeParametreDbs.L4_CREDITS_MIN_STANDARD))
        .thenReturn(80);
    lenient()
        .when(regleBourseService.getValeurAsInteger(TypeParametreDbs.L5_CREDITS_MIN_STANDARD))
        .thenReturn(150);
